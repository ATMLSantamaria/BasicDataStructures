...
// struct HashItem {
//     //Constructor 1
//     HashItem(Content value,std::string key) {
//         value_ = new Content();
//         key_=key;
//     }
//     HashItem(Content * value,std::string key) :value_(value),key_(key) {
//     }
//     HashItem(){
...
//         key_=target.key_;
//         // value_ = new Content(target.value_);
...
class HashTable {
 ...
 HashTable(uint64_t len):array_capacity_(len){
  ...
  uint64_t index_computation(std::string key,uint64_t array_len) {

    uint64_t index = 0;

    index = hash_function(key) % array_len;
    std::cout << hash_function(key) << std::endl;
    std::cout << "index result = " << index << std::endl;

    return index;

  }
  uint64_t hash_function(std::string & key) {
...
/*     if (compute_load_factor() > 0.5d) {
    ...
    int element_index_ = index_computation(target.key_,array_capacity_);
